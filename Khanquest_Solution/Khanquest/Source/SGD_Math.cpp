...
tVector2D Vector2DRotate(tVector2D vec, float fRadians)
{
	...
	float fCosTheta = cos(fRadians);
	float fSinTheta = sin(fRadians);

	tVector2D vRotated;
	vRotated.fX = (fCosTheta * vec.fX) + (fSinTheta * vec.fY);
	vRotated.fY = (-fSinTheta * vec.fX) + (fCosTheta * vec.fY);
...
/*
	...
	NOTE:	This will return the smallest angle between the vectors.
*/
